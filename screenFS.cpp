...
void ScreenFS::apply(const Vertex &vertex)
{
    ...
    Eigen::Vector3f worldPos = gBufferData_->worldPositionBuffer.getBuffer(u, v);
    Eigen::Vector3f normal = gBufferData_->normalBuffer.getBuffer(u, v); // 获取法线数据
    float depth = gBufferData_->zBuffer.getDepth(u, v); // 获取深度数据
    ...
    colorBuffer_->setPixel(x, y, 255, 0, 0, 255);
}
